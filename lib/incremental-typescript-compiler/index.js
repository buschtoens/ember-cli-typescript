...
module.exports = class IncrementalTypescriptCompiler {
  ...
  treeForGeneratedTypes() {
    if (!this._generatedTypesTree) {
      fs.removeSync('types/generated');

      let projectRegistry = this._typeGeneratorRegistryFor(this.project);
      let appDir = `${this._relativeAppRoot()}/app`;
      let trees = [
        projectRegistry.process('app', appDir, appDir),
        projectRegistry.process('tests', 'tests', 'tests', { exclude: ['dummy/**'] })
      ];

      for (let addon of this.addons) {
        let addonRegistry = this._typeGeneratorRegistryFor(addon);
        trees.push(addonRegistry.process('app', 'app', 'app'));
        trees.push(addonRegistry.process('addon', 'addon', 'addon'));
      }

      this._generatedTypesTree = new MergeTrees(trees.filter(Boolean));
    }
