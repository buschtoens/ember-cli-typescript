...
module.exports = class IncrementalTypescriptCompiler {
  ...
  treeForGeneratedTypes() {
    if (!this._generatedTypesTree) {
      ...
      let trees = [];
      trees = trees.concat(projectRegistry.process('app', appDir, appDir));
      trees = trees.concat(projectRegistry.process('tests', 'tests', 'tests', { exclude: ['dummy/**'] }));

      for (let addon of this.addons) {
        let addonRegistry = this._typeGeneratorRegistryFor(addon);
        trees = trees.concat(addonRegistry.process('app', 'app', 'app'));
        trees = trees.concat(addonRegistry.process('addon', 'addon', 'addon'));
      }

      let GeneratedTypesPlugin = require('../type-generation/generated-types-plugin');
      this._generatedTypesTree = new GeneratedTypesPlugin(new MergeTrees(trees.filter(Boolean)), {
